...
const RollingBetaChart: React.FC<RollingBetaChartProps> = ({ data }) => {
  ...
  const allBetas = chartData.flatMap(item => 
    [item.beta_30d, item.beta_90d].filter(val => val !== null && val !== undefined) as number[]
  );
  ...
  // Ensure we have data
  if (!chartData.length) {
    return <div className="flex items-center justify-center h-full">No beta data available</div>;
  }
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <Legend 
          ...
          formatter={(value) => value === 'beta_30d' ? '30-day Beta' : '90-day Beta'}
        />
